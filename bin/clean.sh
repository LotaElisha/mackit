...
EXPORT_LIST_FILE="$HOME/.config/mackit/clean-list.txt"
CURRENT_SECTION=""
...
WHITELIST_WARNINGS=()
if [[ -f "$HOME/.config/mackit/whitelist" ]]; then
    ...
    done < "$HOME/.config/mackit/whitelist"
else
...
safe_clean() {
    ...
    local permission_start=${MACKIT_PERMISSION_DENIED_COUNT:-0}

    local show_scan_feedback=false
    if [[ ${#targets[@]} -gt 20 && -t 1 ]]; then
        show_scan_feedback=true
        stop_section_spinner
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Scanning ${#targets[@]} items..."
    fi
    ...
    if [[ "${MK_DEBUG:-}" == "1" && ${#existing_paths[@]} -gt 0 ]]; then
    ...
    if [[ ${#existing_paths[@]} -gt 10 ]]; then
        ...
        if [[ -t 1 ]]; then MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Scanning items..."; fi
    fi
    ...
    if [[ ${#existing_paths[@]} -gt 3 ]]; then
        ...
        if [[ $dir_count -lt 5 && ${#existing_paths[@]} -gt 20 ]]; then
            if [[ -t 1 && "$show_spinner" == "false" ]]; then
                MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Scanning items..."
                show_spinner=true
            fi
            ...
            if [[ ${#existing_paths[@]} -gt 0 ]]; then
                for path in "${existing_paths[@]}"; do
                    ...
                    if ((${#pids[@]} >= MACKIT_MAX_PARALLEL_JOBS)); then
        ...
        if [[ "$DRY_RUN" != "true" && ${#existing_paths[@]} -gt 0 && -t 1 ]]; then
            MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Cleaning..."
    ...
    local permission_end=${MACKIT_PERMISSION_DENIED_COUNT:-0}
...
start_cleanup() {
    ...
    export MACKIT_CURRENT_COMMAND="clean"
...
# MacKit Cleanup Preview - $(date '+%Y-%m-%d %H:%M:%S')
#
# How to protect files:
# 1. Copy any path below to ~/.config/mackit/whitelist
# 2. Run: mk flush --whitelist
#
# Example:
...
EOF
        ...
        if has_sudo_session; then
            ...
            SYSTEM_CLEAN=false
            echo -e "${GRAY}${ICON_WARNING} System caches need sudo, run ${NC}sudo -v && mk flush --dry-run${GRAY} for full preview${NC}"
            echo ""
        fi
        return
    fi
...
perform_cleanup() {
    ...
    if [[ -z "$EXTERNAL_VOLUME_TARGET" && "${MACKIT_TEST_MODE:-0}" == "1" ]]; then
    ...
    if [[ $total_size_cleaned -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            summary_details+=("Use ${GRAY}mk flush --whitelist${NC} to add protection rules")
        else
