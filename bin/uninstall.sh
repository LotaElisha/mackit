...
readonly MACKIT_UNINSTALL_USER_LC_ALL="${LC_ALL:-}"
readonly MACKIT_UNINSTALL_USER_LANG="${LANG:-}"
...
readonly MACKIT_UNINSTALL_META_CACHE_DIR="$HOME/.cache/mackit"
readonly MACKIT_UNINSTALL_META_CACHE_FILE="$MACKIT_UNINSTALL_META_CACHE_DIR/uninstall_app_metadata_v1"
readonly MACKIT_UNINSTALL_META_CACHE_LOCK="${MACKIT_UNINSTALL_META_CACHE_FILE}.lock"
readonly MACKIT_UNINSTALL_META_REFRESH_TTL=604800 # 7 days
readonly MACKIT_UNINSTALL_SCAN_SPINNER_DELAY_SEC="0.25"
readonly MACKIT_UNINSTALL_INLINE_METADATA_LIMIT=8
readonly MACKIT_UNINSTALL_INLINE_MDLS_TIMEOUT_SEC="0.08"
...
uninstall_resolve_display_name() {
    ...
    if [[ -f "$app_path/Contents/Info.plist" ]]; then
        ...
        if [[ -n "$MACKIT_UNINSTALL_USER_LC_ALL" ]]; then
            md_display_name=$(run_with_timeout 0.04 env LC_ALL="$MACKIT_UNINSTALL_USER_LC_ALL" LANG="$MACKIT_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        elif [[ -n "$MACKIT_UNINSTALL_USER_LANG" ]]; then
            md_display_name=$(run_with_timeout 0.04 env LANG="$MACKIT_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        else
...
uninstall_collect_inline_metadata() {
    ...
    local metadata_date
    metadata_date=$(run_with_timeout "$MACKIT_UNINSTALL_INLINE_MDLS_TIMEOUT_SEC" mdls -name kMDItemLastUsedDate -raw "$app_path" 2> /dev/null || echo "")
    ...
    (
        _refresh_debug() {
            if [[ "${MK_DEBUG:-}" == "1" ]]; then
                ...
                echo "[$ts] DEBUG: [metadata-refresh] $*" >> "${HOME}/.config/mackit/mole_debug_session.log" 2> /dev/null || true
            fi
        }

        ensure_user_dir "$MACKIT_UNINSTALL_META_CACHE_DIR"
        ensure_user_file "$MACKIT_UNINSTALL_META_CACHE_FILE"
        if [[ ! -r "$MACKIT_UNINSTALL_META_CACHE_FILE" ]]; then
            if ! : > "$MACKIT_UNINSTALL_META_CACHE_FILE" 2> /dev/null; then
                _refresh_debug "Cannot create cache file, aborting"
                exit 0
            fi
        fi
        if [[ ! -w "$MACKIT_UNINSTALL_META_CACHE_FILE" ]]; then
        ...
        if ! uninstall_acquire_metadata_lock "$MACKIT_UNINSTALL_META_CACHE_LOCK"; then
        ...
        merged_file=$(mktemp 2> /dev/null) || {
            ...
            uninstall_release_metadata_lock "$MACKIT_UNINSTALL_META_CACHE_LOCK"
            rm -f "$updates_file"
            exit 0
        }
        ...
        ' "$updates_file" "$MACKIT_UNINSTALL_META_CACHE_FILE" > "$merged_file"

        mv "$merged_file" "$MACKIT_UNINSTALL_META_CACHE_FILE" 2> /dev/null || {
            cp "$merged_file" "$MACKIT_UNINSTALL_META_CACHE_FILE" 2> /dev/null || true
            rm -f "$merged_file"
        }
...
scan_applications() {
    ...
    ensure_user_dir "$MACKIT_UNINSTALL_META_CACHE_DIR"
    ensure_user_file "$MACKIT_UNINSTALL_META_CACHE_FILE"
    local cache_source="$MACKIT_UNINSTALL_META_CACHE_FILE"
    ...
    (
        ...
        sleep "$MACKIT_UNINSTALL_SCAN_SPINNER_DELAY_SEC" 2> /dev/null || sleep 1
    ...
    while IFS='|' read -r app_path display_name bundle_id app_mtime cached_mtime cached_size_kb cached_epoch cached_updated_epoch cached_bundle_id cached_display_name; do
        ...
        if [[ $cache_match == false ]]; then
            ...
            if [[ $cache_age -gt $MACKIT_UNINSTALL_META_REFRESH_TTL ]]; then
                needs_refresh=true
            fi
        fi

        if [[ $needs_refresh == true ]]; then
            if [[ $inline_metadata_count -lt $MACKIT_UNINSTALL_INLINE_METADATA_LIMIT ]]; then
    ...
    if [[ -s "$cache_snapshot_file" ]]; then
        if uninstall_acquire_metadata_lock "$MACKIT_UNINSTALL_META_CACHE_LOCK"; then
            mv "$cache_snapshot_file" "$MACKIT_UNINSTALL_META_CACHE_FILE" 2> /dev/null || {
                cp "$cache_snapshot_file" "$MACKIT_UNINSTALL_META_CACHE_FILE" 2> /dev/null || true
...
cleanup() {
    local exit_code="${1:-$?}"
    if [[ "${MACKIT_ALT_SCREEN_ACTIVE:-}" == "1" ]]; then
        leave_alt_screen
        unset MACKIT_ALT_SCREEN_ACTIVE
    fi
...
main() {
    ...
    export MACKIT_CURRENT_COMMAND="uninstall"
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            "--debug")
                ...
                ;;
            "--dry-run" | "-n")
                export MACKIT_DRY_RUN=1
                ;;
            "--whitelist")
                echo "Unknown uninstall option: $arg"
                echo "Whitelist management is currently supported by: mk flush --whitelist / mk turbo --whitelist"
                echo "Use 'mk wipe --help' for supported options."
                exit 1
                ;;
            -*)
                echo "Unknown uninstall option: $arg"
    ...
    while true; do
        unset MACKIT_INLINE_LOADING MACKIT_MANAGED_ALT_SCREEN
        ...
        local prompt_timeout="${MACKIT_UNINSTALL_RETURN_PROMPT_TIMEOUT_SEC:-3}"
