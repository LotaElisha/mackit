...
declare MACKIT_SELECTION_RESULT
declare MACKIT_INSTALLER_SCAN_MAX_DEPTH
...
scan_installers_in_path() {
    local path="$1"
    local max_depth="${MACKIT_INSTALLER_SCAN_MAX_DEPTH:-$INSTALLER_SCAN_MAX_DEPTH_DEFAULT}"
    ...
    while true; do
        ...
        case "$key" in
            ...
            "" | $'\n' | $'\r') # Enter - confirm
                ...
                for ((i = 0; i < total_items; i++)); do
                    if [[ ${selected[i]} == true ]]; then
                        [[ -n "$MACKIT_SELECTION_RESULT" ]] && MACKIT_SELECTION_RESULT+=","
                        MACKIT_SELECTION_RESULT+="$i"
                    fi
                done
                restore_terminal
                return 0
...
delete_selected_installers() {
    ...
    [[ -n "$MACKIT_SELECTION_RESULT" ]] && IFS=',' read -ra selected_indices <<< "$MACKIT_SELECTION_RESULT"
