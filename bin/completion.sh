...
command_names=()
for entry in "${MACKIT_COMMANDS[@]}"; do
    command_names+=("${entry%%:*}")
done
...
emit_fish_completions() {
    ...
    printf '\n'
    printf 'complete -c %s -n "not __fish_mole_no_subcommand" -a bash -d "generate bash completion" -n "__fish_see_subcommand_path shell"\n' "$cmd"
    printf 'complete -c %s -n "not __fish_mole_no_subcommand" -a zsh -d "generate zsh completion" -n "__fish_see_subcommand_path shell"\n' "$cmd"
    printf 'complete -c %s -n "not __fish_mole_no_subcommand" -a fish -d "generate fish completion" -n "__fish_see_subcommand_path shell"\n' "$cmd"
}
...
if [[ $# -eq 0 ]]; then
    ...
    completion_name=""
    if command -v mackit > /dev/null 2>&1; then
        completion_name="mackit"
    elif command -v mk > /dev/null 2>&1; then
        completion_name="mk"
    fi
    ...
    case "$current_shell" in
        ...
        *)
            ...
            echo "  mackit shell <bash|zsh|fish>"
            exit 1
            ;;
    esac

    if [[ -z "$completion_name" ]]; then
        if [[ -f "$config_file" ]] && grep -Eq "(^# MacKit shell completion$|(mackit|mk)[[:space:]]+completion)" "$config_file" 2> /dev/null; then
            if [[ "${MACKIT_DRY_RUN:-0}" == "1" ]]; then
                ...
                temp_file="$(mktemp)"
                grep -Ev "(^# MacKit shell completion$|(mackit|mk)[[:space:]]+completion)" "$config_file" > "$temp_file" || true
        ...
        log_error "mackit not found in PATH, install MacKit before enabling completion"
        exit 1
    fi

    # Check if already installed and normalize to latest line
    if [[ -f "$config_file" ]] && grep -Eq "(mackit|mk)[[:space:]]+completion" "$config_file" 2> /dev/null; then
        if [[ "${MACKIT_DRY_RUN:-0}" == "1" ]]; then
    ...
    # Remove previous MacKit completion lines to avoid duplicates
    if [[ -f "$config_file" ]]; then
        original_mode=""
        original_mode="$(stat -f '%Mp%Lp' "$config_file" 2> /dev/null || true)"
        temp_file="$(mktemp)"
...
complete -F _mackit_completions mackit mk
EOF
        ;;
    zsh)
        printf '#compdef mackit mk\n\n'
        printf '_mackit() {\n'
        ...
        printf '}\n\n'
        printf 'compdef _mackit mackit mk\n'
        ;;
    fish)
        printf '# Completions for mackit\n'
        emit_fish_completions mackit
        printf '\n# Completions for mk (alias)\n'
        emit_fish_completions mk
        ...
        printf '    string match -q -- "shell" (commandline -opc)[1]\n'
        printf 'end\n'
        ;;
    *)
        cat << 'EOF'
Usage: mackit shell [bash|zsh|fish]

Setup shell tab completion for mackit and mk commands.

Auto-install:
  mackit shell              # Auto-detect shell and install
  mackit shell --dry-run    # Preview config changes without writing files

Manual install:
  mackit shell bash         # Generate bash completion script
  mackit shell zsh          # Generate zsh completion script
  mackit shell fish         # Generate fish completion script
...
Examples:
  ...
  # Manual install - Bash
  eval "$(mackit shell bash)"

  # Manual install - Zsh
  eval "$(mackit shell zsh)"

  # Manual install - Fish
  mackit shell fish | source
EOF
        exit 1
        ;;
esac
