...
PAM_SUDO_FILE="${MACKIT_PAM_SUDO_FILE:-/etc/pam.d/sudo}"
PAM_SUDO_LOCAL_FILE="${MACKIT_PAM_SUDO_LOCAL_FILE:-$(dirname "$PAM_SUDO_FILE")/sudo_local}"
...
enable_touchid() {
    ...
    if [[ ! -f "${PAM_SUDO_FILE}.mackit-backup" ]]; then
        if ! sudo cp "$PAM_SUDO_FILE" "${PAM_SUDO_FILE}.mackit-backup" 2> /dev/null; then
