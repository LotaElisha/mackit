#!/bin/bash
# MacKit - Installer for manual installs.
...
resolve_source_dir() {
    if [[ -n "$SOURCE_DIR" && -d "$SOURCE_DIR" && -f "$SOURCE_DIR/mackit" ]]; then
        return 0
    fi
    ...
    if [[ -n "${BASH_SOURCE[0]:-}" && -f "${BASH_SOURCE[0]}" ]]; then
        ...
        if [[ -f "$script_dir/mackit" ]]; then
            SOURCE_DIR="$script_dir"
            return 0
        fi
    fi

    if [[ -n "${CLEAN_SOURCE_DIR:-}" && -d "$CLEAN_SOURCE_DIR" && -f "$CLEAN_SOURCE_DIR/mackit" ]]; then
    ...
    if command -v curl > /dev/null 2>&1; then
        if curl -fsSL --connect-timeout 10 --max-time 60 -o "$tmp/mackit.tar.gz" "$url" 2> /dev/null; then
            if tar -xzf "$tmp/mackit.tar.gz" -C "$tmp" 2> /dev/null; then
                stop_line_spinner

                local extracted_dir
                extracted_dir=$(find "$tmp" -mindepth 1 -maxdepth 1 -type d | head -n 1)

                if [[ -n "$extracted_dir" && -f "$extracted_dir/mackit" ]]; then
    ...
    start_line_spinner "Cloning MacKit source..."
...
# Version helpers
get_source_version() {
    local source_mackit="$SOURCE_DIR/mackit"
    if [[ -f "$source_mackit" ]]; then
        sed -n 's/^VERSION="\(.*\)"$/\1/p' "$source_mackit" | head -n1
    fi
}
...
get_source_commit_hash() {
    ...
    curl -fsSL --connect-timeout 3 \
        "https://api.github.com/repos/lotaelisha/mackit/commits/main" 2> /dev/null |
...
get_latest_release_tag() {
    ...
    tag=$(curl -fsSL --connect-timeout 2 --max-time 3 \
        "https://api.github.com/repos/lotaelisha/mackit/releases/latest" 2> /dev/null |
...
get_installed_version() {
    local binary="$INSTALL_DIR/mackit"
    if [[ -x "$binary" ]]; then
        local version
        version=$("$binary" --version 2> /dev/null | awk '/MacKit version/ {print $NF; exit}')
        if [[ -n "$version" ]]; then
            echo "$version"
        else
...
resolve_install_channel() {
    ...
    if [[ "${MACKIT_EDGE_INSTALL:-}" == "true" ]]; then
        printf 'nightly\n'
        return 0
    fi
...
parse_args() {
    ...
    for i in "${!args[@]}"; do
        ...
        case "$token" in
            latest | main)
                export MACKIT_VERSION="main"
                export MACKIT_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            dev)
                export MACKIT_VERSION="dev"
                export MACKIT_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            [0-9]* | V[0-9]* | v[0-9]*)
                export MACKIT_VERSION="$token"
                version_token="$token"
                unset 'args[$i]'
                ;;
            *)
...
check_requirements() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list mackit > /dev/null 2>&1; then
        local mole_path
        mole_path=$(command -v mackit 2> /dev/null || true)
        local is_homebrew_binary=false

        if [[ -n "$mole_path" && -L "$mole_path" ]]; then
            if readlink "$mole_path" | grep -q "Cellar/mackit"; then
        ...
        if [[ "$is_homebrew_binary" == "true" ]]; then
            ...
            echo -e "${YELLOW}MacKit is installed via Homebrew${NC}"
            echo ""
            echo "Choose one:"
            echo -e "  1. Update via Homebrew: ${GREEN}brew upgrade mackit${NC}"
            echo -e "  2. Switch to manual: ${GREEN}brew uninstall --force mackit${NC} then re-run this"
            echo ""
            exit 1
        else
            log_warning "Cleaning up stale Homebrew installation..."
            brew uninstall --force mackit > /dev/null 2>&1 || true
        fi
    fi
...
install_files() {
    ...
    if [[ -f "$SOURCE_DIR/mackit" ]]; then
        if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
            ...
            maybe_sudo cp "$SOURCE_DIR/mackit" "$INSTALL_DIR/mackit.new"
            maybe_sudo chmod +x "$INSTALL_DIR/mackit.new"
            maybe_sudo mv -f "$INSTALL_DIR/mackit.new" "$INSTALL_DIR/mackit"

            log_success "Installed mackit to $INSTALL_DIR"
        fi
    else
        log_error "mackit executable not found in ${SOURCE_DIR:-unknown}"
        exit 1
    fi

    if [[ -f "$SOURCE_DIR/mo" ]]; then
        if [[ "$source_dir_abs" == "$install_dir_abs" ]]; then
            log_success "mk alias already present"
        else
    ...
    if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
        maybe_sudo sed -i '' "s|SCRIPT_DIR=.*|SCRIPT_DIR=\"$CONFIG_DIR\"|" "$INSTALL_DIR/mackit"
    fi
...
verify_installation() {
    ...
    if [[ -x "$INSTALL_DIR/mackit" ]] && [[ -f "$CONFIG_DIR/lib/core/common.sh" ]]; then

        if "$INSTALL_DIR/mackit" --help > /dev/null 2>&1; then
            return 0
        else
            log_warning "MacKit command installed but may not be working properly"
        fi
    else
...
setup_path() {
    ...
    if [[ "$INSTALL_DIR" != "/usr/local/bin" ]]; then
        ...
        echo ""
        echo "To use mackit from anywhere, add this line to your shell profile:"
...
print_usage_summary() {
    ...
    local message="MacKit ${action} successfully"
...
perform_install() {
    ...
    if [[ "${MACKIT_EDGE_INSTALL:-}" == "true" ]]; then
        ...
        echo ""
        local branch_name="${MACKIT_VERSION:-main}"
...
perform_update() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list mackit > /dev/null 2>&1; then
        ...
        if [[ -f "$SOURCE_DIR/lib/core/common.sh" ]]; then
            ...
            log_error "Cannot update Homebrew-managed MacKit without full installation"
            echo ""
            echo "Please update via Homebrew:"
            echo -e "  ${GREEN}brew upgrade mackit${NC}"
            exit 1
        fi
        exit 0
    fi
    ...
    if [[ -z "$installed_version" ]]; then
        log_warning "MacKit is not currently installed in $INSTALL_DIR. Running fresh installation."
        perform_install
        return
    fi
    ...
    if [[ -z "$target_version" ]]; then
        log_error "Unable to determine the latest MacKit version."
        exit 1
    fi
