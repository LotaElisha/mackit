...
// MacKit body frames (facing right).
var macKitBody = [][]string{
	{
		`     /\_/\`,
		` ___/ o o \`,
		`/___   =-= /`,
...
// Mirror mackit body frames (facing left).
var macKitBodyMirror = [][]string{
	{
		`    /\_/\`,
		`   / o o \___`,
		`  \ =-=   ___\`,
...
// getMacKitFrame renders the animated mackit.
func getMacKitFrame(animFrame int, termWidth int) string {
	macKitWidth := 15
	maxPos := max(termWidth-macKitWidth, 0)
...
func renderHeader(m MetricsSnapshot, errMsg string, animFrame int, termWidth int, catHidden bool) (string, string) {
	...
	// Show cat unless hidden - render mackit centered below header
	var mackit string
	if !catHidden {
		mackit = getMacKitFrame(animFrame, termWidth)
	}
	...
	if errMsg != "" {
		...
		return lipgloss.JoinVertical(lipgloss.Left, headerLine, "", mackit, dangerStyle.Render("ERROR: "+errMsg)), ""
	}
	if mackit == "" {
		return headerLine, ""
	}
	return headerLine, mackit
}
