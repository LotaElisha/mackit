...
func TestRenderHeaderErrorReturnsMacKitOnce(t *testing.T) {
	header, mackit := renderHeader(MetricsSnapshot{}, "boom", 0, 120, false)

	if mackit != "" {
		t.Fatalf("renderHeader() mackit return should be empty on error to avoid duplicate render, got %q", mackit)
	}
	if !strings.Contains(header, "ERROR: boom") {
		t.Fatalf("renderHeader() missing error text, got %q", header)
	}
	if strings.Count(header, "/\\_/\\") != 1 {
		t.Fatalf("renderHeader() should contain one mackit frame in error state, got %d", strings.Count(header, "/\\_/\\"))
	}
}
...
func TestModelViewErrorRendersSingleMacKit(t *testing.T) {
	m := model{
		width:      120,
		height:     40,
		ready:      true,
	...
	if strings.Count(view, "/\\_/\\") != 1 {
		t.Fatalf("model.View() should render one mackit frame in error state, got %d", strings.Count(view, "/\\_/\\"))
	}
}
