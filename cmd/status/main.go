...
func getConfigPath() string {
	...
	return filepath.Join(home, ".config", "mackit", "status_prefs")
}
...
func (m model) View() string {
	...
	header, mackit := renderHeader(m.metrics, m.errMessage, m.animFrame, termWidth, m.catHidden)
	...
	// Combine header, mackit, and cards with consistent spacing
	parts := []string{header}
	if alertBar != "" {
		parts = append(parts, alertBar)
	}
	if mackit != "" {
		parts = append(parts, mackit)
	}
