# MacKit Security Audit

This document describes the security-relevant behavior of the current `main` branch. It is intended as a public description of MacKit's safety boundaries, destructive-operation controls, release integrity signals, and known limitations.

## Executive Summary

MacKit is a local system maintenance tool. Its main risk surface is not remote code execution; it is unintended local damage caused by cleanup, uninstall, optimize, purge, installer cleanup, or other destructive operations.
...
MacKit prioritizes bounded cleanup over aggressive cleanup. When uncertainty exists, the tool should refuse, skip, or require stronger confirmation instead of widening deletion scope.
...
The highest-risk areas in MacKit are:
...
MacKit has explicit protected-path and protected-category logic in addition to root-path blocking.
...
MacKit uses sudo for a subset of system-maintenance paths, but elevated behavior is still bounded by validation and protected-path rules.
...
When sudo is denied or unavailable, MacKit prefers skipping privileged cleanup to forcing execution through unsafe fallback behavior.

## Sensitive Data Exclusions

MacKit is not intended to aggressively delete high-value user data.
...
MacKit exposes multiple safety controls before and during destructive actions:
...
- operation logs are written to `~/Library/Logs/mackit/operations.log` unless disabled with `MO_NO_OPLOG=1`
...
MacKit treats release trust as part of its security posture, not just a packaging detail.
