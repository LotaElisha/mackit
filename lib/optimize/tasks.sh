...
readonly MACKIT_TM_THIN_TIMEOUT=180
readonly MACKIT_TM_THIN_VALUE=9999999999
readonly MACKIT_SQLITE_MAX_SIZE=104857600 # 100MB
...
opt_saved_state_cleanup() {
    if [[ "${MK_DEBUG:-}" == "1" ]]; then
        ...
        debug_operation_detail "Method" "Find and remove .savedState folders older than $MACKIT_SAVED_STATE_AGE_DAYS days"
    ...
    if [[ -d "$state_dir" ]]; then
        ...
        done < <(command find "$state_dir" -type d -name "*.savedState" -mtime "+$MACKIT_SAVED_STATE_AGE_DAYS" -print0 2> /dev/null)
    fi
...
opt_fix_broken_configs() {
    ...
    if [[ -t 1 ]]; then
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Checking preferences..."
...
opt_network_optimization() {
    ...
    if [[ "${MACKIT_DNS_FLUSHED:-0}" == "1" ]]; then
...
opt_sqlite_vacuum() {
    ...
    local spinner_started="false"
    if [[ "${MACKIT_DRY_RUN:-0}" != "1" && -t 1 ]]; then
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Optimizing databases..."
        spinner_started="true"
    fi
    ...
    for pattern in "${db_paths[@]}"; do
        while IFS= read -r db_file; do
            ...
            if [[ "$file_size" -gt "$MACKIT_SQLITE_MAX_SIZE" ]]; then
                skipped=$((skipped + 1))
                continue
            fi
...
opt_launch_services_rebuild() {
    ...
    if [[ -t 1 ]]; then
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Repairing LaunchServices..."
    fi
...
opt_bluetooth_reset() {
    ...
    if [[ -t 1 ]]; then
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Checking Bluetooth..."
        spinner_started="true"
    fi
