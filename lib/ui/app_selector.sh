...
select_apps_for_uninstall() {
    ...
    # - MACKIT_MENU_META_EPOCHS: numeric last_used_epoch per item
    # - MACKIT_MENU_META_SIZEKB: numeric size in KB per item
    # The menu will gracefully fallback if these are unset or malformed.
    if [[ $has_epoch_metadata == true ]]; then
        export MACKIT_MENU_META_EPOCHS="$epochs_csv"
    else
        unset MACKIT_MENU_META_EPOCHS
    fi
    if [[ $has_size_metadata == true ]]; then
        export MACKIT_MENU_META_SIZEKB="$sizekb_csv"
    else
        unset MACKIT_MENU_META_SIZEKB
    fi
    export MACKIT_MENU_FILTER_NAMES="$names_newline"

    # Use paginated menu - result will be stored in MACKIT_SELECTION_RESULT
    ...
    unset MACKIT_MENU_META_EPOCHS MACKIT_MENU_META_SIZEKB MACKIT_MENU_FILTER_NAMES
    # leave MACKIT_MENU_SORT_DEFAULT untouched if user set it globally

    if [[ $exit_code -ne 0 ]]; then
        return 1
    fi

    if [[ -z "$MACKIT_SELECTION_RESULT" ]]; then
    ...
    IFS=',' read -r -a indices_array <<< "$MACKIT_SELECTION_RESULT"
