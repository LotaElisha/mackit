...
paginated_multi_select() {
    ...
    if [[ "${MACKIT_MANAGED_ALT_SCREEN:-}" == "1" || "${MACKIT_MANAGED_ALT_SCREEN:-}" == "true" ]]; then
    ...
    local cursor_pos=0
    local top_index=0
    local sort_mode="${MACKIT_MENU_SORT_MODE:-${MACKIT_MENU_SORT_DEFAULT:-date}}" # date|name|size
    local sort_reverse="${MACKIT_MENU_SORT_REVERSE:-false}"
    ...
    if [[ -n "${MACKIT_MENU_META_EPOCHS:-}" ]]; then
        while IFS= read -r v; do epochs+=("${v:-0}"); done < <(_pm_parse_csv_to_array "$MACKIT_MENU_META_EPOCHS")
        has_metadata="true"
    fi
    if [[ -n "${MACKIT_MENU_META_SIZEKB:-}" ]]; then
        while IFS= read -r v; do sizekb+=("${v:-0}"); done < <(_pm_parse_csv_to_array "$MACKIT_MENU_META_SIZEKB")
        has_metadata="true"
    fi
    if [[ -n "${MACKIT_MENU_FILTER_NAMES:-}" ]]; then
        while IFS= read -r v; do filter_names+=("$v"); done <<< "$MACKIT_MENU_FILTER_NAMES"
    ...
    if [[ -n "${MACKIT_PRESELECTED_INDICES:-}" ]]; then
        local cleaned_preselect="${MACKIT_PRESELECTED_INDICES//[[:space:]]/}"
    ...
    cleanup() {
        trap - EXIT INT TERM
        unset MACKIT_READ_KEY_FORCE_CHAR
        export MACKIT_MENU_SORT_MODE="${sort_mode:-name}"
        export MACKIT_MENU_SORT_REVERSE="${sort_reverse:-false}"
        restore_terminal
    }
    ...
    draw_header() {
        ...
        elif [[ -n "${MACKIT_READ_KEY_FORCE_CHAR:-}" ]]; then
    ...
    handle_filter_char() {
        local char="$1"
        if [[ -z "${MACKIT_READ_KEY_FORCE_CHAR:-}" ]]; then
            return 1
        fi
        if [[ "$char" =~ ^[[:print:]]$ ]]; then
            local char_lower
    ...
    while true; do
        ...
        case "$key" in
            "QUIT")
                if [[ -n "$filter_text" || -n "${MACKIT_READ_KEY_FORCE_CHAR:-}" ]]; then
                    filter_text=""
                    filter_text_lower=""
                    unset MACKIT_READ_KEY_FORCE_CHAR
                    rebuild_view
            ...
            "CHAR:o" | "CHAR:O")
                ...
                ;;
            "CHAR:/" | "CHAR:?")
                if [[ -n "${MACKIT_READ_KEY_FORCE_CHAR:-}" ]]; then
                    unset MACKIT_READ_KEY_FORCE_CHAR
                else
                    export MACKIT_READ_KEY_FORCE_CHAR=1
                fi
                need_full_redraw=true
                ;;
            "DELETE")
            ...
            "ENTER")
                ...
                trap - EXIT INT TERM
                MACKIT_SELECTION_RESULT="$final_result"
                unset MACKIT_READ_KEY_FORCE_CHAR
                export MACKIT_MENU_SORT_MODE="${sort_mode:-name}"
                export MACKIT_MENU_SORT_REVERSE="${sort_reverse:-false}"
                restore_terminal
