...
list_login_items() {
    ...
    if [[ "${MACKIT_TEST_MODE:-0}" == "1" || "${MACKIT_TEST_NO_AUTH:-0}" == "1" ]]; then
        return
    fi
...
get_software_updates() {
    ...
    if [[ -t 1 && -z "${SOFTWAREUPDATE_SPINNER_SHOWN:-}" ]]; then
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Checking system updates..."
...
check_homebrew_updates() {
    ...
    if [[ "$use_cache" == "false" ]]; then
        ...
        if [[ -t 1 ]]; then
            MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Checking Homebrew updates..."
            spinner_started=true
        fi
...
check_mole_update() {
    ...
    # Check if MacKit has updates
    # Auto-detect version from mackit main script
    local current_version
    if [[ -f "${SCRIPT_DIR:-/usr/local/bin}/mackit" ]]; then
        current_version=$(grep '^VERSION=' "${SCRIPT_DIR:-/usr/local/bin}/mackit" 2> /dev/null | head -1 | sed 's/VERSION="\(.*\)"/\1/' || echo "unknown")
    else
    ...
    export MACKIT_UPDATE_AVAILABLE="false"
    ...
    if is_cache_valid "$cache_file"; then
        ...
        if [[ -t 1 ]]; then
            MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Checking MacKit version..."
        fi
        ...
        if command -v curl > /dev/null 2>&1; then
            ...
            curl -fsSL --connect-timeout 3 --max-time 5 https://api.github.com/repos/lotaelisha/mackit/releases/latest 2> /dev/null | grep '"tag_name"' | sed -E 's/.*"v?([^"]+)".*/\1/' > "$temp_version" &
            local curl_pid=$!
    ...
    if [[ -n "$latest_version" && "$current_version" != "$latest_version" ]]; then
        ...
        if [[ "$(printf '%s\n' "$current_version" "$latest_version" | sort -V | head -1)" == "$current_version" ]]; then
            export MACKIT_UPDATE_AVAILABLE="true"
            printf "  ${GRAY}%s${NC} %-12s ${YELLOW}%s${NC}, running %s\n" "$ICON_WARNING" "MacKit" "${latest_version} available" "${current_version}"
        else
            printf "  ${GREEN}✓${NC} %-12s %s\n" "MacKit" "Latest version ${current_version}"
        fi
    else
        printf "  ${GREEN}✓${NC} %-12s %s\n" "MacKit" "Latest version ${current_version}"
    fi
...
check_login_items() {
    ...
    if [[ -t 0 ]]; then
        ...
        if [[ -t 1 ]]; then
            MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Checking login items..."
        fi
...
check_cache_size() {
    ...
    if [[ -t 1 ]]; then
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Scanning cache..."
    fi
