#!/bin/bash
# MacKit - UI Components
# Terminal UI utilities: cursor control, keyboard input, spinners, menus

set -euo pipefail

if [[ -n "${MACKIT_UI_LOADED:-}" ]]; then
    return 0
fi
readonly MACKIT_UI_LOADED=1
...
read_key() {
    ...
    if [[ "${MACKIT_READ_KEY_FORCE_CHAR:-}" == "1" ]]; then
        [[ -z "$key" ]] && {
            echo "ENTER"
            return 0
        }
    ...
    if [[ -t 1 ]]; then
        ...
        (
            ...
            while [[ ! -f "$stop_file" ]]; do
                ...
                printf "\r${MACKIT_SPINNER_PREFIX:-}${BLUE}%s${NC} %s" "$c" "$display_message" >&2 || break
                i=$((i + 1))
                /bin/sleep 0.05
            done
...
has_full_disk_access() {
    ...
    if [[ -n "${MACKIT_HAS_FDA:-}" ]]; then
        if [[ "$MACKIT_HAS_FDA" == "1" ]]; then
            return 0
        elif [[ "$MACKIT_HAS_FDA" == "unknown" ]]; then
            return 2
        else
            return 1
        fi
