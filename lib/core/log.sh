#!/bin/bash
# MacKit - Logging System
# Centralized logging with rotation support

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${MACKIT_LOG_LOADED:-}" ]]; then
    return 0
fi
readonly MACKIT_LOG_LOADED=1
...
readonly LOG_FILE="${HOME}/Library/Logs/mackit/mackit.log"
readonly DEBUG_LOG_FILE="${HOME}/Library/Logs/mackit/mole_debug_session.log"
readonly OPERATIONS_LOG_FILE="${HOME}/Library/Logs/mackit/operations.log"
...
rotate_log_once() {
    ...
    [[ -n "${MACKIT_LOG_ROTATED:-}" ]] && return 0
    export MACKIT_LOG_ROTATED=1
...
log_system_info() {
    ...
    [[ -n "${MACKIT_SYS_INFO_LOGGED:-}" ]] && return 0
    export MACKIT_SYS_INFO_LOGGED=1
    ...
    {
        echo "----------------------------------------------------------------------"
        echo "MacKit Debug Session, $(date '+%Y-%m-%d %H:%M:%S')"
        echo "----------------------------------------------------------------------"
        echo "User: $USER"
