#!/bin/bash
# MacKit - Application Protection
# System critical and data-protected application lists

set -euo pipefail

if [[ -n "${MACKIT_APP_PROTECTION_LOADED:-}" ]]; then
    return 0
fi
readonly MACKIT_APP_PROTECTION_LOADED=1

_MACKIT_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[[ -z "${MACKIT_BASE_LOADED:-}" ]] && source "$_MACKIT_CORE_DIR/base.sh"
...
#
# In uninstall mode (MACKIT_UNINSTALL_MODE=1), only system-critical components are protected.
...
should_protect_path() {
    ...
    if [[ "$path" =~ /Library/Containers/([^/]+) ]] || [[ "$path" =~ /Library/Group\ Containers/([^/]+) ]]; then
        ...
        if [[ "${MACKIT_UNINSTALL_MODE:-0}" != "1" ]] && should_protect_data "$bundle_id"; then
            return 0
        fi
    fi
    ...
    case "$path" in
        */Library/Preferences/com.apple.dock.plist | */Library/Preferences/com.apple.finder.plist)
            return 0
            ;;
        # Protect MacKit's own runtime logs so cleanup cannot delete its active log targets.
        */Library/Logs/mackit | */Library/Logs/mackit/ | */Library/Logs/mackit/*)
            return 0
            ;;
    ...
    # This catches things like /Users/lotaelisha/Library/Caches/Claude when pattern is *Claude*
    # In uninstall mode, only check system-critical bundles (user explicitly chose to uninstall)
    if [[ "${MACKIT_UNINSTALL_MODE:-0}" == "1" ]]; then
