#!/bin/bash
# MacKit - File Operations
# Safe file and directory manipulation with validation

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${MACKIT_FILE_OPS_LOADED:-}" ]]; then
    return 0
fi
readonly MACKIT_FILE_OPS_LOADED=1

# Error codes for removal operations
readonly MACKIT_ERR_SIP_PROTECTED=10
readonly MACKIT_ERR_AUTH_FAILED=11
readonly MACKIT_ERR_READONLY_FS=12

# Ensure dependencies are loaded
_MACKIT_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -z "${MACKIT_BASE_LOADED:-}" ]]; then
    # shellcheck source=lib/core/base.sh
    source "$_MACKIT_CORE_DIR/base.sh"
fi
if [[ -z "${MACKIT_LOG_LOADED:-}" ]]; then
    # shellcheck source=lib/core/log.sh
    source "$_MACKIT_CORE_DIR/log.sh"
fi
if [[ -z "${MACKIT_TIMEOUT_LOADED:-}" ]]; then
    # shellcheck source=lib/core/timeout.sh
    source "$_MACKIT_CORE_DIR/timeout.sh"
fi
...
safe_remove() {
    ...
    if [[ $rm_exit -eq 0 ]]; then
        ...
        log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "REMOVED" "$path" "$size_human"
        return 0
    else
        # Check if it's a permission error
        if [[ "$error_msg" == *"Permission denied"* ]] || [[ "$error_msg" == *"Operation not permitted"* ]]; then
            MACKIT_PERMISSION_DENIED_COUNT=${MACKIT_PERMISSION_DENIED_COUNT:-0}
            MACKIT_PERMISSION_DENIED_COUNT=$((MACKIT_PERMISSION_DENIED_COUNT + 1))
            export MACKIT_PERMISSION_DENIED_COUNT
            debug_log "Permission denied: $path, may need Full Disk Access"
            log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "FAILED" "$path" "permission denied"
        else
            [[ "$silent" != "true" ]] && log_error "Failed to remove: $path"
            log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "FAILED" "$path" "error"
        fi
        return 1
    fi
}
...
safe_remove_symlink() {
    ...
    if [[ $rm_exit -eq 0 ]]; then
        log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "REMOVED" "$path" "symlink"
        return 0
    else
        log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "FAILED" "$path" "symlink removal failed"
        return 1
    fi
}
...
safe_sudo_remove() {
    ...
    case "$output" in
        *"Operation not permitted"*)
            log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "FAILED" "$path" "sip/mdm protected"
            return "$MACKIT_ERR_SIP_PROTECTED"
            ;;
        *"Read-only file system"*)
            log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "FAILED" "$path" "readonly filesystem"
            return "$MACKIT_ERR_READONLY_FS"
            ;;
        *"Sorry, try again"* | *"incorrect passphrase"* | *"incorrect credentials"*)
            log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "FAILED" "$path" "auth failed"
            return "$MACKIT_ERR_AUTH_FAILED"
            ;;
        *)
            log_error "Failed to remove, sudo: $path"
            log_operation "${MACKIT_CURRENT_COMMAND:-clean}" "FAILED" "$path" "sudo error"
            return 1
            ;;
    esac
}
...
get_path_size_kb() {
    ...
    if [[ "$size" =~ ^[0-9]+$ ]]; then
        ...
        [[ "${MK_DEBUG:-}" == "1" ]] && debug_log "get_path_size_kb: Failed to get size for $path (returned: $size)"
        echo "0"
    fi
}
...
diagnose_removal_failure() {
    ...
    case "$exit_code" in
        "$MACKIT_ERR_SIP_PROTECTED")
            reason="protected by macOS (SIP/MDM)"
            ;;
        "$MACKIT_ERR_AUTH_FAILED")
            ...
            if [[ -f "$touchid_file" ]] && grep -q "pam_tid.so" "$touchid_file" 2> /dev/null; then
                ...
                suggestion="Try 'mackit touchid' to enable fingerprint auth"
            fi
            ;;
        "$MACKIT_ERR_READONLY_FS")
        ...
        *)
            ...
            if [[ -f "$touchid_file" ]] && grep -q "pam_tid.so" "$touchid_file" 2> /dev/null; then
                ...
                suggestion="Try 'mackit touchid' or check with 'ls -l'"
            fi
            ;;
    esac
