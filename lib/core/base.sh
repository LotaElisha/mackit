#!/bin/bash
# MacKit - Base Definitions and Utilities
# Core definitions, constants, and basic utility functions used by all modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${MACKIT_BASE_LOADED:-}" ]]; then
    return 0
fi
readonly MACKIT_BASE_LOADED=1
...
# ============================================================================
readonly MACKIT_TEMP_FILE_AGE_DAYS=7       # Temp file retention (days)
readonly MACKIT_ORPHAN_AGE_DAYS=30         # Orphaned data retention (days)
readonly MACKIT_MAX_PARALLEL_JOBS=15       # Parallel job limit
readonly MACKIT_MAIL_DOWNLOADS_MIN_KB=5120 # Mail attachment size threshold
readonly MACKIT_MAIL_AGE_DAYS=30           # Mail attachment retention (days)
readonly MACKIT_LOG_AGE_DAYS=7             # Log retention (days)
readonly MACKIT_CRASH_REPORT_AGE_DAYS=7    # Crash report retention (days)
readonly MACKIT_SAVED_STATE_AGE_DAYS=30    # Saved state retention (days) - increased for safety
readonly MACKIT_TM_BACKUP_SAFE_HOURS=48    # TM backup safety window (hours)
readonly MACKIT_MAX_DS_STORE_FILES=500     # Max .DS_Store files to clean per scan
readonly MACKIT_MAX_ORPHAN_ITERATIONS=100  # Max iterations for orphaned app data scan
...
detect_architecture() {
    if [[ -n "${MACKIT_ARCH_CACHE:-}" ]]; then
        echo "$MACKIT_ARCH_CACHE"
        return 0
    fi

    if [[ "$(uname -m)" == "arm64" ]]; then
        export MACKIT_ARCH_CACHE="Apple Silicon"
    else
        export MACKIT_ARCH_CACHE="Intel"
    fi
    echo "$MACKIT_ARCH_CACHE"
}
...
get_darwin_major() {
    if [[ -n "${MACKIT_DARWIN_MAJOR_CACHE:-}" ]]; then
        echo "$MACKIT_DARWIN_MAJOR_CACHE"
        return 0
    fi
    ...
    export MACKIT_DARWIN_MAJOR_CACHE="$major"
    echo "$major"
}
...
get_optimal_parallel_jobs() {
    ...
    if [[ -z "${MACKIT_CPU_CORES_CACHE:-}" ]]; then
        export MACKIT_CPU_CORES_CACHE=$(sysctl -n hw.ncpu 2> /dev/null || echo 4)
    fi
    local cpu_cores="$MACKIT_CPU_CORES_CACHE"
...
get_invoking_user() {
    if [[ -n "${_MACKIT_INVOKING_USER_CACHE:-}" ]]; then
        echo "$_MACKIT_INVOKING_USER_CACHE"
        return 0
    fi
    ...
    export _MACKIT_INVOKING_USER_CACHE="$user"
    echo "$user"
}
...
get_brand_name() {
    ...
    if [[ -z "${MACKIT_IS_CHINESE_SYSTEM:-}" ]]; then
        local sys_lang
        sys_lang=$(defaults read -g AppleLanguages 2> /dev/null | grep -o 'zh-Hans\|zh-Hant\|zh' | head -1 || echo "")
        if [[ -n "$sys_lang" ]]; then
            export MACKIT_IS_CHINESE_SYSTEM="true"
        else
            export MACKIT_IS_CHINESE_SYSTEM="false"
        fi
    fi

    local is_chinese="${MACKIT_IS_CHINESE_SYSTEM}"
...
declare -a MACKIT_TEMP_FILES=()
declare -a MACKIT_TEMP_DIRS=()

# Create tracked temporary file
create_temp_file() {
    local temp
    temp=$(mktemp "${TMPDIR:-/tmp}/mackit.XXXXXX") || return 1
...
create_temp_dir() {
    local temp
    temp=$(mktemp -d "${TMPDIR:-/tmp}/mackit.XXXXXX") || return 1
...
cleanup_temp_files() {
    ...
    local file
    if [[ ${#MACKIT_TEMP_FILES[@]} -gt 0 ]]; then
        for file in "${MACKIT_TEMP_FILES[@]}"; do
            [[ -f "$file" ]] && rm -f "$file" 2> /dev/null || true
        done
    fi

    if [[ ${#MACKIT_TEMP_DIRS[@]} -gt 0 ]]; then
        for file in "${MACKIT_TEMP_DIRS[@]}"; do
...
start_section_spinner() {
    ...
    stop_inline_spinner || true
    if [[ -t 1 ]]; then
        MACKIT_SPINNER_PREFIX="  " start_inline_spinner "$message"
    fi
}
...
is_ansi_supported() {
    if [[ -n "${MACKIT_ANSI_SUPPORTED_CACHE:-}" ]]; then
        return "$MACKIT_ANSI_SUPPORTED_CACHE"
    fi

    # Check if running in interactive terminal
    if ! [[ -t 1 ]]; then
        export MACKIT_ANSI_SUPPORTED_CACHE=1
        return 1
    fi
    ...
    case "$TERM" in
        xterm* | vt100 | vt220 | screen* | tmux* | ansi | linux | rxvt* | konsole*)
            export MACKIT_ANSI_SUPPORTED_CACHE=0
            return 0
            ;;
        dumb | unknown)
            export MACKIT_ANSI_SUPPORTED_CACHE=1
