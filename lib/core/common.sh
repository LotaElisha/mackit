#!/bin/bash
# MacKit - Common Functions Library
# Main entry point that loads all core modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${MACKIT_COMMON_LOADED:-}" ]]; then
    return 0
fi
readonly MACKIT_COMMON_LOADED=1
...
source "$_MACKIT_CORE_DIR/file_ops.sh"
source "$_MACKIT_CORE_DIR/help.sh"
source "$_MACKIT_CORE_DIR/ui.sh"
source "$_MACKIT_CORE_DIR/app_protection.sh"

# Load sudo management if available
if [[ -f "$_MACKIT_CORE_DIR/sudo.sh" ]]; then
    source "$_MACKIT_CORE_DIR/sudo.sh"
fi
...
update_via_homebrew() {
    ...
    # Upgrade MacKit
    if [[ -t 1 ]]; then
        start_inline_spinner "Upgrading MacKit..."
    else
        echo "Upgrading MacKit..."
    fi

    brew upgrade mackit > "$temp_upgrade" 2>&1 &
    local upgrade_pid=$!
    ...
    if echo "$upgrade_output" | grep -q "already installed"; then
        ...
        installed_version=$(brew list --versions mackit 2> /dev/null | awk '{print $2}')
        [[ -z "$installed_version" ]] && installed_version=$(mo --version 2> /dev/null | awk '/MacKit version/ {print $3; exit}')
        echo ""
        ...
        local new_version
        new_version=$(brew list --versions mackit 2> /dev/null | awk '{print $2}')
        [[ -z "$new_version" ]] && new_version=$(mo --version 2> /dev/null | awk '/MacKit version/ {print $3; exit}')
        echo ""
    ...
    rm -f "$HOME/.cache/mackit/version_check" "$HOME/.cache/mackit/update_message" 2> /dev/null || true
}
