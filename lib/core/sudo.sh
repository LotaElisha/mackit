...
request_sudo_access() {
    ...
    if [[ "$is_gui_mode" == true ]]; then
        ...
        local password
        password=$(osascript -e "display dialog \"$prompt_msg\" default answer \"\" with title \"MacKit\" with icon caution with hidden answer" -e 'text returned of result' 2> /dev/null)
...
# Global state
MACKIT_SUDO_KEEPALIVE_PID=""
MACKIT_SUDO_ESTABLISHED="false"
...
ensure_sudo_session() {
    ...
    if has_sudo_session && [[ "$MACKIT_SUDO_ESTABLISHED" == "true" ]]; then
        return 0
    fi
    ...
    if [[ -n "$MACKIT_SUDO_KEEPALIVE_PID" ]]; then
        _stop_sudo_keepalive "$MACKIT_SUDO_KEEPALIVE_PID"
        MACKIT_SUDO_KEEPALIVE_PID=""
    fi
    ...
    # Start keepalive
    MACKIT_SUDO_KEEPALIVE_PID=$(_start_sudo_keepalive)

    MACKIT_SUDO_ESTABLISHED="true"
    return 0
}
