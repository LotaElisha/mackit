...
_MACKIT_MANAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_MACKIT_MANAGE_DIR/../core/common.sh"
source "$_MACKIT_MANAGE_DIR/../ui/menu_simple.sh"

# Config file paths
readonly WHITELIST_CONFIG_CLEAN="$HOME/.config/mackit/whitelist"
readonly WHITELIST_CONFIG_OPTIMIZE="$HOME/.config/mackit/whitelist_optimize"
readonly WHITELIST_CONFIG_OPTIMIZE_LEGACY="$HOME/.config/mackit/whitelist_checks"
...
MacKit updates check|check_mole_update|update_check
...
${GRAY}Edit: ${display_config}${NC}"
    ...
    if [[ ${#selected_cache_patterns[@]} -gt 0 ]]; then
        ...
        local IFS=','
        export MACKIT_PRESELECTED_INDICES="${preselected_indices[*]}"
    else
        unset MACKIT_PRESELECTED_INDICES
    fi

    MACKIT_SELECTION_RESULT=""
    ...
    if [[ -n "$MACKIT_SELECTION_RESULT" ]]; then
        local -a selected_indices
        IFS=',' read -ra selected_indices <<< "$MACKIT_SELECTION_RESULT"
