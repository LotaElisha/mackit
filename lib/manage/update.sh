...
ask_for_updates() {
    ...
    if [[ -n "${MACKIT_UPDATE_AVAILABLE:-}" && "${MACKIT_UPDATE_AVAILABLE}" == "true" ]]; then
        has_updates=true
    fi
    ...
    if [[ "${MACKIT_UPDATE_AVAILABLE:-}" == "true" ]]; then
        echo -ne "${YELLOW}Update MacKit now?${NC} ${GRAY}Enter confirm / ESC cancel${NC}: "
...
perform_updates() {
    # Only handle MacKit updates here; Homebrew/App Store/macOS are manual (tips shown in ask_for_updates)
    local updated_count=0
    local total_count=0

    if [[ -n "${MACKIT_UPDATE_AVAILABLE:-}" && "${MACKIT_UPDATE_AVAILABLE}" == "true" ]]; then
        echo -e "${BLUE}Updating MacKit...${NC}"
        local mackit_bin="${SCRIPT_DIR}/../../mackit"
        [[ ! -f "$mackit_bin" ]] && mackit_bin=$(command -v mackit 2> /dev/null || echo "")

        if [[ -x "$mackit_bin" ]]; then
            if "$mackit_bin" update 2>&1 | grep -qE "(Updated|latest version)"; then
                echo -e "${GREEN}${ICON_SUCCESS}${NC} MacKit updated"
                reset_mackit_cache
                updated_count=$((updated_count + 1))
            else
                echo -e "${RED}✗${NC} MacKit update failed"
            fi
        else
            echo -e "${RED}✗${NC} MacKit executable not found"
        fi
        echo ""
        total_count=1
    fi
