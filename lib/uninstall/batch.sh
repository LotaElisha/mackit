...
[[ -z "${MACKIT_COMMON_LOADED:-}" ]] && source "$SCRIPT_DIR/lib/core/common.sh"
...
batch_uninstall_applications() {
    ...
    trap 'stop_inline_spinner 2>/dev/null; _cleanup_sudo_keepalive; unset MACKIT_UNINSTALL_MODE; echo ""; _restore_uninstall_traps; return 130' INT TERM
    ...
    export MACKIT_UNINSTALL_MODE=1
    ...
    if [[ ${#sudo_apps[@]} -gt 0 && "${MACKIT_DRY_RUN:-0}" != "1" ]]; then
        ...
        if [[ -z "$reason" ]]; then
            if [[ "$is_brew_cask" == "true" && -n "$cask_name" ]]; then
                ...
                if brew_uninstall_cask "$cask_name" "$app_path"; then
                    ...
                    # where brew still reports the app as installed after MacKit
    ...
    if [[ $brew_apps_removed -gt 0 && "${MACKIT_DRY_RUN:-0}" != "1" ]]; then
        (
