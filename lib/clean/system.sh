...
clean_deep_system() {
    ...
    if sudo test -d "/Library/Caches" 2> /dev/null; then
        while IFS= read -r -d '' file; do
            ...
            \( -name "*.cache" -mtime "+$MACKIT_TEMP_FILE_AGE_DAYS" \) -o \
            \( -name "*.tmp" -mtime "+$MACKIT_TEMP_FILE_AGE_DAYS" \) -o \
            \( -name "*.log" -mtime "+$MACKIT_LOG_AGE_DAYS" \) \
    ...
    for tmp_dir in "${sys_temp_dirs[@]}"; do
        if sudo find "$tmp_dir" -maxdepth 1 -type f -mtime "+${MACKIT_TEMP_FILE_AGE_DAYS}" -print -quit 2> /dev/null | grep -q .; then
            if safe_sudo_find_delete "$tmp_dir" "*" "${MACKIT_TEMP_FILE_AGE_DAYS}" "f"; then
                tmp_cleaned=1
            fi
        fi
    done
    ...
    if sudo find "/Library/Logs/DiagnosticReports" -maxdepth 1 -type f -mtime "+$MACKIT_CRASH_REPORT_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
        safe_sudo_find_delete "/Library/Logs/DiagnosticReports" "*" "$MACKIT_CRASH_REPORT_AGE_DAYS" "f" || true
    fi
    stop_section_spinner
    log_success "System crash reports"
    start_section_spinner "Cleaning system logs..."
    if sudo find "/private/var/log" -maxdepth 3 -type f \( -name "*.log" -o -name "*.gz" -o -name "*.asl" \) -mtime "+$MACKIT_LOG_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
        safe_sudo_find_delete "/private/var/log" "*.log" "$MACKIT_LOG_AGE_DAYS" "f" || true
        safe_sudo_find_delete "/private/var/log" "*.gz" "$MACKIT_LOG_AGE_DAYS" "f" || true
        safe_sudo_find_delete "/private/var/log" "*.asl" "$MACKIT_LOG_AGE_DAYS" "f" || true
    fi
    stop_section_spinner
    ...
    for third_party_log_dir in "${third_party_log_dirs[@]}"; do
        if sudo test -d "$third_party_log_dir" 2> /dev/null; then
            if sudo find "$third_party_log_dir" -maxdepth 5 -type f -mtime "+$MACKIT_LOG_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
                if safe_sudo_find_delete "$third_party_log_dir" "*" "$MACKIT_LOG_AGE_DAYS" "f"; then
                    third_party_logs_cleaned=1
                fi
            fi
        fi
    done
    if sudo find "/Library/Logs" -maxdepth 1 -type f -name "adobegc.log" -mtime "+$MACKIT_LOG_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
    ...
    safe_sudo_find_delete "$diag_base" "*" "$MACKIT_LOG_AGE_DAYS" "f" || true
    safe_sudo_find_delete "$diag_base" "*.tracev3" "30" "f" || true
    safe_sudo_find_delete "/private/var/db/DiagnosticPipeline" "*" "$MACKIT_LOG_AGE_DAYS" "f" || true
    stop_section_spinner
    log_success "System diagnostic logs"

    start_section_spinner "Cleaning power logs..."
    safe_sudo_find_delete "/private/var/db/powerlog" "*" "$MACKIT_LOG_AGE_DAYS" "f" || true
    stop_section_spinner
    ...
    for volume in "${backup_volumes[@]}"; do
        ...
        if [[ -d "$backupdb_dir" ]]; then
            while IFS= read -r inprogress_file; do
                ...
                if [[ $hours_old -lt $MACKIT_TM_BACKUP_SAFE_HOURS ]]; then
                    continue
                fi
                local size_kb
