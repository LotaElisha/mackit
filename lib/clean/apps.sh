...
readonly ORPHAN_AGE_THRESHOLD=${ORPHAN_AGE_THRESHOLD:-${MACKIT_ORPHAN_AGE_DAYS:-30}}
readonly CLAUDE_VM_ORPHAN_AGE_THRESHOLD=${MACKIT_CLAUDE_VM_ORPHAN_AGE_DAYS:-7}
...
clean_ds_store_tree() {
    ...
    while IFS= read -r -d '' ds_file; do
        ...
        if [[ $file_count -ge $MACKIT_MAX_DS_STORE_FILES ]]; then
            break
        fi
...
scan_installed_apps() {
    ...
    local cache_file="$HOME/.cache/mackit/installed_apps_cache"
        ...
        if [[ ${#file_patterns[@]} -gt 0 ]]; then
            ...
            for item_path in "${file_patterns[@]}"; do
                ...
                for match in "${matches[@]}"; do
                    ...
                    if [[ $iteration_count -gt $MACKIT_MAX_ORPHAN_ITERATIONS ]]; then
                        break
                    fi
