...
_clean_mail_downloads() {
    local mail_age_days=${MACKIT_MAIL_AGE_DAYS:-}
    ...
    for target_path in "${mail_dirs[@]}"; do
        if [[ -d "$target_path" ]]; then
            ...
            local min_kb="${MACKIT_MAIL_DOWNLOADS_MIN_KB:-}"
            if ! [[ "$min_kb" =~ ^[0-9]+$ ]]; then
                min_kb=5120
            fi
...
clean_edge_old_versions() {
    ...
    local -a app_paths
    if [[ -n "${MACKIT_EDGE_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$MACKIT_EDGE_APP_PATHS"
    else
        app_paths=(
...
clean_support_app_data() {
    local support_age_days="${MACKIT_SUPPORT_CACHE_AGE_DAYS:-30}"
...
clean_app_caches() {
    ...
    local precise_size_limit="${MACKIT_CONTAINER_CACHE_PRECISE_SIZE_LIMIT:-64}"
...
external_volume_root() {
    printf '%s\n' "${MACKIT_EXTERNAL_VOLUMES_ROOT:-/Volumes}"
}
...
clean_application_support_logs() {
    ...
    local cleaned_count=0
    local found_any=false
    local size_timeout_seconds="${MACKIT_APP_SUPPORT_ITEM_SIZE_TIMEOUT_SEC:-0.4}"
