...
check_tcc_permissions() {
    [[ -t 1 ]] || return 0
    local permission_flag="$HOME/.cache/mackit/permissions_granted"
    ...
    if [[ "$needs_permission_check" == "true" ]]; then
        ...
        read -r
        MACKIT_SPINNER_PREFIX="" start_inline_spinner "Requesting permissions..."
...
clean_service_worker_cache() {
    ...
    if [[ $cleaned_size -gt 0 ]]; then
        ...
        if [[ "$spinner_was_running" == "true" ]]; then
            MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Scanning browser Service Worker caches..."
        fi
    fi
}
...
project_cache_has_indicators() {
    local dir="$1"
    local max_depth="${2:-5}"
    local indicator_timeout="${MACKIT_PROJECT_CACHE_DISCOVERY_TIMEOUT:-2}"
    [[ -d "$dir" ]] || return 1
...
discover_project_cache_roots() {
    ...
    for root in "${MACKIT_PURGE_DEFAULT_SEARCH_PATHS[@]}"; do
        [[ -d "$root" ]] && roots+=("$root")
    done

    while IFS= read -r root; do
        [[ -d "$root" ]] && roots+=("$root")
    done < <(mole_purge_read_paths_config "$HOME/.config/mackit/purge_paths")
...
scan_project_cache_root() {
    local root="$1"
    local output_file="$2"
    local scan_timeout="${MACKIT_PROJECT_CACHE_SCAN_TIMEOUT:-6}"
    [[ -d "$root" ]] || return 0
