...
clean_homebrew() {
    ...
    local brew_cache_file="${HOME}/.cache/mackit/brew_last_cleanup"
    ...
    if [[ -t 1 ]]; then
        if [[ "$skip_cleanup" == "true" ]]; then
            MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Homebrew autoremove (cleanup skipped)..."
        else
            MACKIT_SPINNER_PREFIX="  " start_inline_spinner "Homebrew cleanup and autoremove..."
        fi
    fi
