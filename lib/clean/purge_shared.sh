...
if [[ -n "${MACKIT_PURGE_SHARED_LOADED:-}" ]]; then
    return 0
fi
readonly MACKIT_PURGE_SHARED_LOADED=1

# Canonical purge targets (heavy project build artifacts).
readonly MACKIT_PURGE_TARGETS=(
    "node_modules"
    "target"        # Rust, Maven
...
readonly MACKIT_PURGE_DEFAULT_SEARCH_PATHS=(
    "$HOME/www"
    "$HOME/dev"
    "$HOME/Projects"
    "$HOME/GitHub"
...
readonly MACKIT_PURGE_MONOREPO_INDICATORS=(
    "lerna.json"
    "pnpm-workspace.yaml"
    "nx.json"
    "rush.json"
...
readonly MACKIT_PURGE_PROJECT_INDICATORS=(
    "package.json"
    "Cargo.toml"
    "go.mod"
    "pyproject.toml"
...
readonly MACKIT_PURGE_QUICK_HINT_EXCLUDED_TARGETS=(
    "bin"
    "vendor"
)
...
mole_purge_is_project_root() {
    ...
    for indicator in "${MACKIT_PURGE_MONOREPO_INDICATORS[@]}"; do
        if [[ -e "$dir/$indicator" ]]; then
            return 0
        fi
    done

    for indicator in "${MACKIT_PURGE_PROJECT_INDICATORS[@]}"; do
        if [[ -e "$dir/$indicator" ]]; then
            return 0
        fi
    done
...
mole_purge_quick_hint_target_names() {
    ...
    for target in "${MACKIT_PURGE_TARGETS[@]}"; do
        is_excluded=false
        for excluded in "${MACKIT_PURGE_QUICK_HINT_EXCLUDED_TARGETS[@]}"; do
...
mole_purge_read_paths_config() {
    local config_file="${1:-$HOME/.config/mackit/purge_paths}"
