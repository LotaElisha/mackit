...
readonly PURGE_TARGETS=("${MACKIT_PURGE_TARGETS[@]}")
...
readonly DEFAULT_PURGE_SEARCH_PATHS=("${MACKIT_PURGE_DEFAULT_SEARCH_PATHS[@]}")

# Config file for custom purge paths.
readonly PURGE_CONFIG_FILE="$HOME/.config/mackit/purge_paths"
...
readonly MONOREPO_INDICATORS=("${MACKIT_PURGE_MONOREPO_INDICATORS[@]}")
readonly PROJECT_INDICATORS=("${MACKIT_PURGE_PROJECT_INDICATORS[@]}")
...
write_purge_config() {
    ...
    local tmp_file
    tmp_file=$(mktemp_file "mackit-purge-paths") || return 1
...
save_discovered_paths() {
    local -a paths=("$@")
    write_purge_config "# MacKit Purge Paths - Auto-discovered project directories
...
scan_purge_targets() {
    ...
    local stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/mackit"
...
clean_project_artifacts() {
    ...
    local dry_run_mode="${MACKIT_DRY_RUN:-0}"
    for idx in "${selected_indices[@]}"; do
