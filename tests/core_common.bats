...
@test "log_info prints message and appends to log file" {
    ...
    local log_file="$HOME/Library/Logs/mackit/mole.log"
    [[ -f "$log_file" ]]
...
rm -rf "$HOME/Library/Logs/mackit"
log_operation "clean" "REMOVED" "/tmp/example" "1KB"
EOF
    [ "$status" -eq 0 ]

    local oplog="$HOME/Library/Logs/mackit/operations.log"
    [[ -f "$oplog" ]]
    grep -Fq "[clean] REMOVED /tmp/example (1KB)" "$oplog"
}

@test "should_protect_path protects MacKit runtime logs" {
    result="$(
        HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" bash --noprofile --norc -c \
            'source "$PROJECT_ROOT/lib/core/common.sh"; should_protect_path "$HOME/Library/Logs/mackit/operations.log" && echo protected || echo not-protected'
    )"
    [ "$result" = "protected" ]
}
