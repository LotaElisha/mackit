...
EOF
	...
	run env HOME="$HOME" "$PROJECT_ROOT/mole" --version
	[ "$status" -eq 0 ]
	[[ "$output" == *"MacKit version $expected_version"* ]]
...
set -euo pipefail
HOME="$(mktemp -d)"
export HOME MACKIT_TEST_MODE=1 MACKIT_SKIP_MAIN=1
...
@test "mk turbo command is recognized" {
	run bash -c "grep -q '\"optimize\")' '$PROJECT_ROOT/mole'"
	[ "$status" -eq 0 ]
}

@test "mk scan binary is valid" {
...
@test "mk flush --debug creates debug log file" {
	mkdir -p "$HOME/.config/mole"
	run env HOME="$HOME" TERM="xterm-256color" MACKIT_TEST_MODE=1 MK_DEBUG=1 "$PROJECT_ROOT/mole" clean --dry-run
	[ "$status" -eq 0 ]
	MOLE_OUTPUT="$output"

	DEBUG_LOG="$HOME/Library/Logs/mackit/mole_debug_session.log"
	[ -f "$DEBUG_LOG" ]

	run grep "MacKit Debug Session" "$DEBUG_LOG"
	[ "$status" -eq 0 ]

	[[ "$MOLE_OUTPUT" =~ "Debug session log saved to" ]]
}

@test "mk flush without debug does not show debug log path" {
	mkdir -p "$HOME/.config/mole"
	run env HOME="$HOME" TERM="xterm-256color" MACKIT_TEST_MODE=1 MK_DEBUG=0 "$PROJECT_ROOT/mole" clean --dry-run
	[ "$status" -eq 0 ]

	[[ "$output" != *"Debug session log saved to"* ]]
}

@test "mk flush --debug logs system info" {
	mkdir -p "$HOME/.config/mole"
	run env HOME="$HOME" TERM="xterm-256color" MACKIT_TEST_MODE=1 MK_DEBUG=1 "$PROJECT_ROOT/mole" clean --dry-run
	[ "$status" -eq 0 ]
...
@test "mk flush --help includes external volume option" {
	run env HOME="$HOME" "$PROJECT_ROOT/mole" clean --help
	[ "$status" -eq 0 ]
	[[ "$output" == *"--external PATH"* ]]
}

@test "mk flush --external accepts canonicalized custom root" {
...
@test "mk scan --json outputs valid JSON with expected fields" {
...
@test "mk scan --json entries contain required fields" {
	if [[ ! -x "${ANALYZE_BIN:-}" ]]; then
		skip "analyze binary not available (go not installed?)"
	fi
...
@test "mk scan --json path reflects target directory" {
	if [[ ! -x "${ANALYZE_BIN:-}" ]]; then
		skip "analyze binary not available (go not installed?)"
	fi
...
@test "mk matrix --json outputs valid JSON with expected fields" {
...
@test "mk matrix --json cpu section has expected structure" {
	if [[ ! -x "${STATUS_BIN:-}" ]]; then
		skip "status binary not available (go not installed?)"
	fi
...
@test "mk matrix --json memory section has expected structure" {
	if [[ ! -x "${STATUS_BIN:-}" ]]; then
		skip "status binary not available (go not installed?)"
	fi
...
@test "mk matrix --json piped to stdout auto-detects JSON mode" {
	if [[ ! -x "${STATUS_BIN:-}" ]]; then
		skip "status binary not available (go not installed?)"
	fi
