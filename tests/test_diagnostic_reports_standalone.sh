...
TMP_EMPTY=$(mktemp -d 2> /dev/null || mktemp -d -t mackit-test 2> /dev/null || echo "")
[[ -z "$TMP_EMPTY" ]] && TMP_EMPTY="/tmp/mackit-test-$$" && mkdir -p "$TMP_EMPTY"
...
TMP_DIAG=$(mktemp -d 2> /dev/null || mktemp -d -t mackit-diag 2> /dev/null || echo "/tmp/mackit-diag-$$")
TMP_APP=$(mktemp -d 2> /dev/null || mktemp -d -t mackit-app 2> /dev/null || echo "/tmp/mackit-app-$$")
...
TMP_DIAG2=$(mktemp -d 2> /dev/null || mktemp -d -t mackit-diag2 2> /dev/null || echo "/tmp/mackit-diag2-$$")
TMP_APP2=$(mktemp -d 2> /dev/null || mktemp -d -t mackit-app2 2> /dev/null || echo "/tmp/mackit-app2-$$")
