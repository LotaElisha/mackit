...
@test "perform_updates handles Homebrew success and MacKit update" {
    run bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/manage/update.sh"
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Updating MacKit"* ]]
    [[ "$output" == *"MacKit updated"* ]]
...
process_install_output() {
    ...
    if ! printf '%s\n' "$output" | grep -Eq "Updated to latest version|Already on latest version"; then
        ...
        if [[ -z "$new_version" ]]; then
            new_version=$(command -v mk > /dev/null 2>&1 && mk --version 2> /dev/null | awk 'NR==1 && NF {print $NF}' || echo "")
        fi
        if [[ -z "$new_version" ]]; then
...
output="Installing MacKit...
◎ MacKit installed successfully, version 1.23.1"
...
curl() {
  ...
  if [[ -n "$out" ]]; then
    cat > "$out" << 'INSTALLER'
#!/usr/bin/env bash
echo "MacKit installed successfully, version $CURRENT_VERSION"
INSTALLER
    return 0
  fi
...
@test "update_mole with --nightly uses installer path and passes MACKIT_VERSION=main" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" CURRENT_VERSION="$CURRENT_VERSION" PATH="$HOME/fake-bin:/usr/bin:/bin" TERM="dumb" bash --noprofile --norc << 'EOF'
set -euo pipefail
curl() {
  local out=""
  ...
  if [[ -n "$out" ]]; then
    cat > "$out" << 'INSTALLER'
#!/usr/bin/env bash
echo "INSTALLER_MACKIT_VERSION=${MACKIT_VERSION:-}"
echo "MacKit installed successfully, version ${MACKIT_VERSION:-unknown}"
INSTALLER
    return 0
  fi
...
EOF
    ...
    [[ "$output" == *"INSTALLER_MACKIT_VERSION=main"* ]]
...
@test "get_homebrew_latest_version prefers brew outdated verbose target version" {
    run bash --noprofile --norc <<'EOF'
set -euo pipefail
MACKIT_TEST_MODE=1 MACKIT_SKIP_MAIN=1 source "$PROJECT_ROOT/mole"

brew() {
  if [[ "${1:-}" == "outdated" ]]; then
    echo "lotaelisha/tap/mole (1.29.0) < 1.31.0"
    return 0
  fi
  if [[ "${1:-}" == "info" ]]; then
    echo "==> lotaelisha/tap/mole: stable 9.9.9 (bottled)"
    return 0
  fi
  return 0
}
...
brew() {
  ...
  if [[ "${1:-}" == "info" ]]; then
    echo "==> lotaelisha/tap/mole: stable 1.31.1 (bottled), HEAD"
    return 0
  fi
  return 0
}
