...
run_clean_dry_run() {
    ...
    run env HOME="$HOME" MACKIT_TEST_MODE=1 PATH="$test_path" \
        "$PROJECT_ROOT/mole" clean --dry-run
}

@test "mk flush --dry-run skips system cleanup in non-interactive mode" {
    set_mock_sudo_uncached
    run_clean_dry_run
    [ "$status" -eq 0 ]
    [[ "$output" == *"Dry Run Mode"* ]]
...
@test "mk flush --dry-run includes system preview when sudo is cached" {
    set_mock_sudo_cached
    run_clean_dry_run
    [ "$status" -eq 0 ]
    [[ "$output" == *"system preview included"* ]]
...
@test "mk flush --dry-run shows hint when sudo is not cached" {
    set_mock_sudo_uncached
    run_clean_dry_run
    [ "$status" -eq 0 ]
    [[ "$output" == *"sudo -v"* ]]
...
@test "mk flush --dry-run reports user cache without deleting it" {
    mkdir -p "$HOME/Library/Caches/TestApp"
    echo "cache data" > "$HOME/Library/Caches/TestApp/cache.tmp"

    run env HOME="$HOME" MACKIT_TEST_MODE=1 "$PROJECT_ROOT/mole" clean --dry-run
    [ "$status" -eq 0 ]
...
@test "mk flush --dry-run reports stale login item without deleting it" {
...
@test "mk flush --dry-run does not export duplicate targets across sections" {
    ...
    run env HOME="$HOME" MACKIT_TEST_MODE=0 "$PROJECT_ROOT/mole" clean --dry-run
    [ "$status" -eq 0 ]
...
@test "mk flush honors whitelist entries" {
...
@test "mk flush honors whitelist entries with $HOME literal" {
    mkdir -p "$HOME/Library/Caches/WhitelistedApp"
    echo "keep me" > "$HOME/Library/Caches/WhitelistedApp/data.tmp"
...
@test "mk flush protects Maven repository by default" {
