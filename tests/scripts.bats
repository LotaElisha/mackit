...
@test "check.sh script exists and is valid" {
    ...
    run bash -c "grep -q 'MacKit Check' '$PROJECT_ROOT/scripts/check.sh'"
    [ "$status" -eq 0 ]
}
...
@test "test.sh script exists and is valid" {
    ...
    run bash -c "grep -q 'MacKit Test Runner' '$PROJECT_ROOT/scripts/test.sh'"
    [ "$status" -eq 0 ]
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Raycast: MacKit Clean | Alfred keyword: clean"* ]]
    [[ "$output" == *"Raycast: MacKit Status | Alfred keyword: status"* ]]
    ...
    run grep -q '^# @raycast.title MacKit Clean$' "$clean_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title MacKit Uninstall$' "$uninstall_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title MacKit Optimize$' "$optimize_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title MacKit Analyze$' "$analyze_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title MacKit Status$' "$status_script"
    [ "$status" -eq 0 ]

    run grep -q '^# @raycast.description Deep system cleanup with MacKit$' "$clean_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description Uninstall applications with MacKit$' "$uninstall_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description System health checks and optimization$' "$optimize_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description Disk space analysis with MacKit$' "$analyze_script"
    [ "$status" -eq 0 ]
...
@test "install.sh supports dev branch installs" {
    ...
    [ "$status" -eq 0 ]
    run bash -c "grep -q 'MACKIT_VERSION=\"dev\"' '$PROJECT_ROOT/install.sh'"
    [ "$status" -eq 0 ]
}
