...
if command -v go > /dev/null 2>&1; then
    GO_TEST_CACHE="${MACKIT_GO_TEST_CACHE:-/tmp/mackit-go-build-cache}"
...
if bash -n mackit && bash -n bin/clean.sh && bash -n bin/optimize.sh; then
...
if [[ "$(uname -s)" != "Darwin" ]]; then
    ...
    # Skip if Homebrew mackit is installed (install.sh will refuse to overwrite)
    install_test_home=""
    if command -v brew > /dev/null 2>&1 && brew list mackit &> /dev/null; then
        printf "${GREEN}${ICON_SUCCESS} Installation test skipped, Homebrew${NC}\n"
    else
        install_test_home="$(mktemp -d /tmp/mackit-test-home.XXXXXX 2> /dev/null || true)"
        if [[ -z "$install_test_home" ]]; then
            install_test_home="/tmp/mackit-test-home"
    ...
    if [[ -z "$install_test_home" ]]; then
        ...
        MO_NO_OPLOG=1 \
        ./install.sh --prefix /tmp/mackit-test > /dev/null 2>&1; then
        if [[ -f "/tmp/mackit-test/mackit" ]]; then
    ...
    MO_NO_OPLOG=1 safe_remove "/tmp/mackit-test" true || true
