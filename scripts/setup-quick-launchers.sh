#!/bin/bash
# Create Raycast script commands and Alfred keywords for MacKit (clean + uninstall).
...
LAUNCHER_COMMAND_SPECS=(
    "clean|MacKit Clean|Deep system cleanup with MacKit|Run MacKit clean"
    "uninstall|MacKit Uninstall|Uninstall applications with MacKit|Uninstall apps via MacKit"
    "optimize|MacKit Optimize|System health checks and optimization|System health and optimization"
    "analyze|MacKit Analyze|Disk space analysis with MacKit|Disk space analysis"
    "status|MacKit Status|Live system status dashboard|Live system dashboard"
)
...
detect_mo() {
    if command -v mo > /dev/null 2>&1; then
        command -v mo
    elif command -v mackit > /dev/null 2>&1; then
        command -v mackit
    else
        log_error "MacKit not found. Install it first via Homebrew or ./install.sh."
        exit 1
    fi
}
...
create_raycast_commands() {
    ...
    for entry in "${LAUNCHER_COMMAND_SPECS[@]}"; do
        ...
        write_raycast_script "$dir/mackit-${subcommand}.sh" "$title" "$description" "$mo_bin" "$subcommand"
    done
...
create_alfred_workflow() {
    ...
    for entry in "${LAUNCHER_COMMAND_SPECS[@]}"; do
        ...
        bundle="fun.lotaelisha.mackit.${subcommand}"
        keyword="${subcommand}"
...
main() {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  MacKit Quick Launchers"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

    local mo_bin
    mo_bin="$(detect_mo)"
    log_step "Detected MacKit binary at: ${mo_bin}"
