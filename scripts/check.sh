#!/bin/bash
# Code quality checks for MacKit.
...
echo -e "${BLUE}=== MacKit Check, ${MODE} ===${NC}\n"

SHELL_FILES=$(find . -type f \( -name "*.sh" -o -name "mackit" \) \
    -not -path "./.git/*" \
...
if [[ "$MODE" == "format" ]]; then
    ...
    if command -v goimports > /dev/null 2>&1; then
        ...
        goimports -w -local github.com/lotaelisha/MacKit ./cmd
...
if command -v shellcheck > /dev/null 2>&1; then
    if shellcheck mackit bin/*.sh lib/*/*.sh scripts/*.sh; then
...
if ! bash -n mackit; then
    echo -e "${RED}${ICON_ERROR} Syntax check failed, mackit${NC}\n"
    exit 1
fi
